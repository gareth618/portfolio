...
Except for HTML, which is full of indentations, the prose-oriented files should word-wrap. On file save, all available StyleLint, ESLint, and Prettier fixes should be applied, in this particular order. I want the editor to remain as minimalistic as possible, so I don't need minimaps, sticky-scrolls, and so on in my life. From GitLens I only make use of CodeLens --- that is, mainly, displaying onto each line of code who authored it.
